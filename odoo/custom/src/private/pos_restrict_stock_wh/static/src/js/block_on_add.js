...
function locLabelFromRow(r, lid) {
    return String(
        r?.complete_name || r?.display_name || r?.location || r?.location_name || lid
    );
}
...
async function getAllowedChildLocationIds(env) {
    ...
    if (!offlineLike()) {
        try {
            ...
            const cfg =
                (
                    await rpc("/web/dataset/call_kw/pos.config/read", {
                        model: "pos.config",
                        method: "read",
            ...
            const ptype =
                (
                    await rpc("/web/dataset/call_kw/stock.picking.type/read", {
            ...
            const children = (Array.isArray(ids) ? ids : [])
                .map(Number)
            ...
            warn("allowed_locs RPC failed", e);
        }
    }
...
function readWhereFromLSLoose(pos, productId) {
    ...
    let rows = [];
    const pushArr = (a) => {
        if (Array.isArray(a) && a.length) rows = rows.concat(a);
    };
    ...
    try {
        pushArr(OFFDB?.readLSWhere?.(pos, productId));
    } catch {}
...
async function getWhereRowsAll(env, productId) {
    ...
    const HAS_CTX =
        OFFDB &&
        typeof OFFDB.buildCtx === "function" &&
        typeof OFFDB.keyWhere === "function";

    if (offlineLike()) {
        if (HAS_CTX) {
            const ctx = OFFDB.buildCtx(pos);
            let rows = await safeIDBGetJSON(
                "where_all",
                OFFDB.keyWhere(ctx, productId)
            );
    ...
    try {
        const rows = await env.services.rpc(
            "/pos_restrict_stock_wh/where_available_all",
            {
...
async function getWhereRows(env, productId) {
    ...
    if (offlineLike()) {
        if (HAS_CTX) {
            ...
            if (!Array.isArray(rows) || rows.length === 0)
                rows = OFFDB?.readLSWhere?.(pos, productId) || [];
...
async function getWhereRowsAny(env, productId) {
    ...
    let b = [];
    try {
        b = await getWhereRowsAll(env, productId);
    } catch (e) {
        warn("getWhereRowsAll lanzó; seguimos con local", e);
    }
    const seen = new Set();
    const out = [];
    for (const r of [...(a || []), ...(b || [])]) {
...
function setReservedFromOrder(pos) {
    ...
    if (order) {
        ...
        for (const l of lines) {
            ...
            const q =
                (l.get_quantity ? Number(l.get_quantity()) : Number(l.qty || 0)) || 0;
...
function getSessionId(pos) {
    return (
        ...
        (Array.isArray(pos?.config?.current_session_id)
            ? pos.config.current_session_id[0]
            : null) ||
        "0"
    );
}
const lowKey = (pos, pid) =>
    baseKey(pos) + "/low_ping/s" + String(getSessionId(pos)) + "/" + String(pid);
...
function maybeLowStockToast(env, product, remainingExact) {
    ...
    if (!(remainingExact > 0 && remainingExact <= thr)) {
        try {
            localStorage.removeItem(lowKey(pos, product.id));
        } catch {}
        return;
    }
    ...
    if (should) {
        env.services.notification?.add?.(
            _t(
                "Stock bajo: quedan %s unidades de %s",
                fmt(remainingExact),
...
function rowOnHand(r) {
    ...
    const hasR =
        Object.prototype.hasOwnProperty.call(r, "reserved") ||
...
async function getRestrictFlag(env) {
    ...
    try {
        const recs = await env.services.rpc("/web/dataset/call_kw/pos.config/read", {
            model: "pos.config",
            method: "read",
            args: [[cfg.id], ["restrict_out_of_stock"]],
            kwargs: {},
        });
...
async function offerAltSourceAndMode(env, product, qty) {
    const allowedIds = (await getAllowedChildLocationIds(env))
        .map(Number)
        .filter(Number.isFinite);
    ...
    for (const r of rowsAny || []) {
        ...
        if (freeEff > 0) {
            ...
            const label = `${locLabelFromRow(r, lid)} — ${_t("Disponible")}: ${fmt(
                freeEff
            )} ${_t("unidades")}`;
            items.push({
                id: lid,
                label,
                item: {id: lid, name: locLabelFromRow(r, lid)},
            });
        }
    }
    ...
    if (!items.length) {
        await env.services.popup.add(ConfirmPopup, {
            ...
            body: _t(
                "No hay ninguna ubicación con stock disponible (ni en caché offline)."
            ),
...
async function canAddProductNow(env, product, qtyToAdd, label = "CHECK") {
    ...
    const reservedSessionExact = getSessionReservedInAllowed(
        pos,
        product.id,
        allowedSet
    );
    const reservedPersistedExact = persistedReservedInAllowed(
        pos,
        product.id,
        allowedSet
    );
    ...
    if (remainingExact <= 0 || need > remainingExact) {
        return {
            ...
            reserved: fmt(alreadyReservedExact),
            remainingExact,
            allowedExact: allowedOnHandExact,
            reservedExact: alreadyReservedExact,
        };
    }
    return {
        ok: true,
...
patch(Orderline.prototype, {
    ...
    export_as_JSON() {
        ...
        if (this.pos_src_location_id)
            json.pos_src_location_id = this.pos_src_location_id;
        if (this.pos_fulfillment_mode)
            json.pos_fulfillment_mode = this.pos_fulfillment_mode;
        return json;
    },
    init_from_JSON(json) {
            ...
            try {
                ...
                if (!chk.ok) {
                    ...
                    try {
                        ...
                        try {
                            if (line) {
                                if (typeof line.setSourceLocation === "function")
                                    line.setSourceLocation(alt.loc);
                                else line.pos_src_location_id = alt.loc?.id || null;
                                if (typeof line.setFulfillmentMode === "function")
                                    line.setFulfillmentMode(alt.mode);
                                else line.pos_fulfillment_mode = alt.mode || null;
                            }
                        } catch (e) {
                            warn("set meta (ProductScreen) failed", e);
                        }
                    } finally {
...
const __orig_add_product__ =
    (Order &&
        Order.prototype &&
        (Order.prototype.add_product || Order.prototype.addProduct)) ||
    null;
...
if (__orig_add_product__ && !Order.prototype.__prsw_add_product_patched__) {
    Order.prototype.add_product = async function (product, options = {}) {
        ...
        if (!options.__prsw_checked && !this.pos.__prsw_skip_recheck_guard) {
            try {
                const chk = await canAddProductNow(
                    this.pos.env,
                    product,
                    qty,
                    "PRE(Order)"
                ...
                if (!chk.ok) {
                    const alt = await askShipFromOtherWarehouse(
                        this.pos.env,
                        product,
                        chk
                    );
                    ...
                    if (alt.requireClient) {
                        await this.pos.env.services.popup.add(ConfirmPopup, {
                            ...
                            body: _t(
                                "Para envío a domicilio asigna un cliente con dirección y vuelve a añadir el producto."
                            ),
                            confirmText: _t("Aceptar"),
                        });
                        return;
                    ...
                    const res2 = await __orig_add_product__.apply(this, [
                        product,
                        {
                            ...options,
                            __prsw_checked: true,
                    ...
                    try {
                        ...
                        warn("set meta (Order) failed", e);
                    }
                    return res2; // evita re-entrada
                }
            } catch (e) {
                warn("Order.add_product guard failed; allowing add", e);
            }
        }
        ...
        try {
            ...
            const optLoc = Number.isFinite(Number(options.__prsw_source_location_id))
                ? Number(options.__prsw_source_location_id)
                : null;
            ...
            try {
                if (line) {
                    if (optLoc && !line.pos_src_location_id) {
                        if (typeof line.setSourceLocation === "function")
                            line.setSourceLocation({id: optLoc});
                        else line.pos_src_location_id = optLoc;
                    }
                    if (options.__prsw_fulfillment_mode && !line.pos_fulfillment_mode) {
                        if (typeof line.setFulfillmentMode === "function")
                            line.setFulfillmentMode(options.__prsw_fulfillment_mode);
                        else
                            line.pos_fulfillment_mode = options.__prsw_fulfillment_mode;
                    }
                }
            } catch (e) {
                warn("apply meta from options failed", e);
            }

            if (product?.id && qty > 0) {
                incReservedByLoc(pos, product.id, effLoc, qty);
                try {
                    const chkAfter = await canAddProductNow(
                        pos.env,
                        product,
                        0,
                        "AFTER"
                    ...
                    if (remainingNow === 0 && SHOW_LAST_UNIT_TOAST) {
                        ...
                        remainingNow > 0 &&
                        remainingNow <= LOW_STOCK_THRESHOLD
                    ) {
                        maybeLowStockToast(pos.env, product, remainingNow);
                    }
                } catch (e) {
                    warn("post-add toast failed", e);
                }
            }
            return res;
        } finally {
...
if (!Orderline.prototype.__prsw_set_quantity_patched__) {
    ...
    Orderline.prototype.set_quantity = async function (q, keep_price) {
        const cur = this.get_quantity
            ? Number(this.get_quantity())
            : Number(this.qty || 0);
        const next = Number(q);
        ...
        try {
            ...
            warn("set_quantity overlay/toast failed", e);
        }
        return res;
    };
    ...
    Order.prototype.remove_orderline = function (line) {
        try {
            ...
            const qty = line?.get_quantity
                ? Number(line.get_quantity())
                : Number(line?.qty || 0);
            const defRoot = getDefaultSrcId(this.pos);
            const effLoc = line?.pos_src_location_id || defRoot;
            if (product?.id && qty > 0)
                incReservedByLoc(this.pos, product.id, effLoc, -qty);
        } catch (e) {
            warn("remove_orderline overlay adjust failed", e);
        }
