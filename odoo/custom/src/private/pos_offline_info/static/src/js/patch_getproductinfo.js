...
patch(PosStore.prototype, {
    async getProductInfo(product, quantity=1) {
        ...
        const cacheSnap = await idbGet(key) || { byProduct:{}, ts:0, version:1 };
        
       
        const mem = (this.offlineInfo?.byProduct?.[product.id]?.info) || null;
       
        const idb_cache = (cacheSnap?.byProduct?.[product.id]?.info) || null;
        
      
        if (!safeOnline()) {
            let info = harden(mem || idb_cache || {
        ...
        try {
            ...
            const prev = cacheSnap.byProduct[product.id] || {};
            cacheSnap.byProduct[product.id] = { ...prev, info: harden(res) };
            cacheSnap.ts = Date.now();
            
            await idbSet(key, cacheSnap); 
            
            this.offlineInfo = cacheSnap;
            return res;
        } catch (e) {
